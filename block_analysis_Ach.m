...
% Find time indices of each block
...
hold on;
p2 = plot(time,Ach_z,'Color', RGB(2,:), 'LineWidth',0.7);
box off;
yl = ylim;
p3 = yline(threshold,'r--');
legend([p1, p2, p3], {'Pupil size (Z-scored)', 'ACh (Z-scored)', 'Pupil threshold'});

% Shade each block where pupil_state == 1
maskRows = blocks(:,3) == 1;
...
xlabel('Time (s)');
ylabel('Z-score');
title('ACh trace during identified brain states');
exportgraphics(gcf, fullfile('figures','figure2_normAch.jpg'), 'Resolution', 300);
...
for block = 1:height(blocks_T)
    ...
    blocks_T.nt_data_z{block} = Ach_z(s:e);
end
...
save("Ach_results.mat", 'off_mean_corr','on_mean_corr', 'diff_real','p_state_corr_perm', '-append');
...
title('ACh Correlation during ON and OFF brain states')
box off;
hold off;
exportgraphics(gcf, fullfile('figures','figure3_corrAch.jpg'), 'Resolution', 300);
...
title('Mean ACh Trace During State Transitions')
box off
exportgraphics(gcf, fullfile('figures','figure4_transitionsAch.jpg'), 'Resolution', 300);
